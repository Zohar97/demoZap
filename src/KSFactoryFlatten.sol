...
contract KSPool is IKSPool, ERC20Permit, ReentrancyGuard {
  ...
  function getReserves() external view override returns (uint112 _reserve0, uint112 _reserve1) {
    _reserve0 = reserve0;
    _reserve1 = reserve1;
  }
  ...
  function verifyBalance(
    ...
    require(balance0Adjusted.mul(balance1Adjusted) >= beforeReserve0.mul(beforeReserve1), 'KS: K');
  }
    ...
    if (feeOn) {
      if (_kLast != 0) {
        ...
        if (_tmp / _vReserve0 == _kLast) {
          ...
          collectedFee0 = _vReserve0.sub(MathExt.sqrt(_kLast.div(_vReserve1).mul(_vReserve0)));
        }
